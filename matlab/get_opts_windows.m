function opts = get_opts_windows(opts, dataset, varargin)

if nargin == 1
    opts = process_opts_windows(opts);
    return;
end
...
ip.addParameter('lastLayer', false); % if true, freeze the pretrained model
...
function opts = process_opts_windows(opts)
